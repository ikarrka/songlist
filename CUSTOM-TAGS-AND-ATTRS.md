...
| `key` | `"Em"`, `"F# >> G"` | **2 (только при открытии)** | Показывается в шапке как `span.song-key` и изменяется кнопками `−/+`, если содержит простую тональность (`C`, `F#`, `Am`, `C#m` и т.п.). Если у `.accordion` есть атрибут `keyrender`, то при рендере `span.song-key` сразу показывается с учётом числового `transpose`; сам атрибут `key` не изменяется. Если одновременно есть `key` и `keycustom`, кнопки изменяют `key`. Сложные значения вроде `F# >> G` пропускаются. |
| `keycustom` | `"D"`, `"Am"` | **2 (только при открытии)** | Резервное имя тональности. Используется и изменяется только если атрибут `key` отсутствует. |
