...
| `key` | `"Em"`, `"F# >> G"` | **2 (только при открытии)** | Показывается в шапке как `span.song-key` и изменяется кнопками `−/+`, если содержит простую тональность (`C`, `F#`, `Am`, `C#m` и т.п.). Если у `.accordion` есть атрибут `keyrender` и числовой `transpose` ≠ 0 (и нет `keycustom`), то при рендере `span.song-key` сразу показывается с учётом `transpose` и со стилем как у `keycustom` (зелёный, bold, 1.2em); сам атрибут `key` не изменяется. Если одновременно есть `key` и `keycustom`, кнопки изменяют `key`. Сложные значения вроде `F# >> G` пропускаются. |
| `keycustom` | `"D"`, `"Am"` | **2 (только при открытии)** | Резервное имя тональности. Используется и изменяется только если атрибут `key` отсутствует. Если `keycustom` задан — `keyrender`/`transpose` на отображение не влияют. |
...
| `transpose` | `"2"` или `"-3"` | **2 лениво** | Автотранспонирование песни сразу при её раскрытии. В [initTransposeForAccordion](file:///d:/source/songlist/music.js#L1108-L1195) N раз «кликает» по `−/+` кнопкам транспонирования. При изменении транспозиции изображения `img[imagetype="scores"]` переключаются на подходящий вариант, если он задан. После ручного изменения появляется кнопка `⌂`, возвращающая песню в начальное состояние, заданное HTML-атрибутами. |
...
|---|---|---|---|
| `data-src` | `"shine.synth.1.png"` (**без** `images/` префикса) | **Фаза 2 лениво** — после клика на аккордеон | Обычная картинка. Для `imagetype="scores"` исходное значение сохраняется и используется как ссылка `original`, если для текущей транспозиции нет отдельного варианта. |
| `data-src-transpose-plus-N` | `"ddt.osen.plus2.png"` | **Фаза 2 лениво** | Альтернативная картинка для транспозиции `+N`, например `data-src-transpose-plus-2` для `transpose="2"`. |
| `data-src-transpose-minus-N` | `"ddt.osen.minus5.png"` | **Фаза 2 лениво** | Альтернативная картинка для транспозиции `-N`, например `data-src-transpose-minus-5` для `transpose="-5"`. |
| `imagetype` | `"scores"` или `"customImage"` | Тот же момент | Фильтр, чтобы «лениво» грузить **только** картинки с этими двумя типами. Для `"scores"` при транспозиции выбирается `data-src-transpose-plus-N` или `data-src-transpose-minus-N`. Если варианта нет, сохраняется сообщение «Notes removed due to key mismatch» со ссылкой `original`. |
